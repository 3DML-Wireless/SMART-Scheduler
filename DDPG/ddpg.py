...
class DDPG(object):
    ...
    def select_action(self, s_t, decay_epsilon=True):
        # proto action
        action = to_numpy(
            self.actor(to_tensor(s_t, gpu_used=self.gpu_used, gpu_0=self.gpu_ids[0])),gpu_used=self.gpu_used
        ).squeeze(0)
        # print('action without noise is:', action)
        action += self.is_training * max(self.epsilon, 0) * self.random_process.sample() ## add noise
        # print('action add noise is:', action)
        action = np.clip(action, -1., 1.) # limit value [-1,1]
